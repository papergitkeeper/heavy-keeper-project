...
public:
	...
	struct node {int x; string y;} t[10005];
	static int cmp(node i,node j) {return i.x>j.x;}
    void get_heavy_hitters(int threshold, vector<pair<string, int>> & results)
    {
        ...
        for (int i = 0; i < bucket_num; ++i) 
            for (int j = 0; j < MAX_VALID_COUNTER; ++j) 
            {
                ...
                t[cnt].x=val; t[cnt++].y=string((const char*)&key, 4);
            }
        std::sort(t,t+cnt,cmp);
        for (int i = 0; i < threshold; i++) results.push_back(make_pair(t[i].y, t[i].x));
    }
